...
int main(int argc, char** argv) {
    ...
    auto window = glfwCreateWindow(640, 480, "Tutorial11", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window)) {
        auto trScale = glm::scale(glm::mat4(1.0F), 
                glm::vec3(
                    static_cast<float> (std::sin(t * 0.1F)), 
                    static_cast<float> (std::sin(t * 0.1F)),
                    static_cast<float> (std::sin(t * 0.1F))));

        auto trTrans = glm::translate(glm::mat4(1.0F), glm::vec3(static_cast<float> (std::sin(t)), 0.0F, 0.0F));
        auto trRotate = glm::rotate(glm::mat4(1.0F), t, glm::vec3(1.0F, 1.0F, 1.0F));

        auto trModel = trScale * trRotate * trTrans;
