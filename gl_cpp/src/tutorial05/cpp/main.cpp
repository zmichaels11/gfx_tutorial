/**
 * Tutorial05 - Uniforms (OpenGL 4.5)
 * 
 * Draws a Triangle in the center of the screen using shaders.
 * The triangle is then scaled by a uniform passed per frame.
 * Uses OpenGL 4.5
 */
...
namespace {
    ...
    const std::string VERTEX_SHADER = 
        ...
        "void main() {\n"
        "  gl_Position = vec4(uScale * position, 1.0);\n"
        "}";
...
int main(int argc, char** argv) {
    ...
    auto window = glfwCreateWindow(640, 480, "Tutorial05", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glProgramUniform3f(
            program, uScale, 
            static_cast<float> (std::sin(t) * 0.5 + 0.5),
            static_cast<float> (std::cos(t) * 0.5 + 0.5),
            1.0F);
